...
#from agents.our_ddpg.utils1 import ReplayBuffer
from agents.our_ddpg.preprocessor import Preprocessor

from agents.our_ddpg.PER.MAMCwithRCandExpl import AACHER
from agents.our_ddpg.loggers import Logger
from agents.our_ddpg.PER import original_buffer
from agents.our_ddpg.PER import PER_buffer
...
def objective(numstations):
    ...
    scenario = "basic"  # basic,convergence
    ...
    run = wandb.init(project="my-awesome-project-2", tags=wtags, name=f"Test+{nWifi}+PER+RC+a10c10")
   ...
   #     "her_k": her_k
    ...
    #Then we use the OpenAI implementation
        ...
        if prioritized_replay_beta_iters is None:
            prioritized_replay_beta_iters = steps_per_ep
        ...
        with tqdm.trange(1, steps_per_ep+1) as t:
            for step in t: #
              ...
              #  replay_buffer.add(state, action, next_state, reward, done)  # inserting action between -1 and 1
                ...
                if time_step >= args.start_timesteps:
                # Assuming your policy is an instance of AACHER
                  #  policy.train(replay_buffer, prioritized=True, beta_value=0.4, args.batch_size)
                    policy.train(replay_buffer, prioritized=True, beta_value=0.4, batch_size=args.batch_size)
