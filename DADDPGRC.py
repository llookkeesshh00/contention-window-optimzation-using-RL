...
from agents.our_ddpg.DADDPGRC import DDPG
...
if __name__ == "__main__":
    ...
    for nWifi in [15]:
        ...
        run = wandb.init(project="my-awesome-project", tags=wtags, name="DARC-CW20-STA15-Hyper-convergence")
        ...
        parser.add_argument("--expl_noise", default=0.35)  # Std of Gaussian exploration noise
        parser.add_argument("--batch_size", default=32, type=int)  # Batch size for both actor and critic
        parser.add_argument("--discount", default=0.8227)  # Discount factor
        parser.add_argument("--tau", default=0.0032)  # Target network update rate
        parser.add_argument("--policy_noise", default=0.2)  # Noise added to target policy during critic update
        parser.add_argument("--noise_clip", default=0.29)  # Range to clip target policy noise
        ...
        real_max_action = 6  # actions are scaled to be in range from 0 to 6
        stateSize = state_dim
                ...
                for step in t:
                    ...
                    if step > 300:
                        ...
                        logger.log_round(state, reward, episode_reward, info, loss,
                                         np.mean(not_processed_state, axis=0)[0], episode * steps_per_ep + step)
