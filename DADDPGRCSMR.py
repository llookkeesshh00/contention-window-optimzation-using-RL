...
if __name__ == "__main__":
	...
	for nWifi in [25]:
		...
		run = wandb.init(project="my-awesome-project", tags = wtags,name="DASMR-CW20-STA25-Hyper")
		...
		parser.add_argument("--expl_noise", default=0.35) #0.1         # Std of Gaussian exploration noise
		parser.add_argument("--batch_size", default=32, type=int) # 256      # Batch size for both actor and critic
		parser.add_argument("--discount", default=0.8227) #0.9             # Discount factor
		parser.add_argument("--tau", default=0.0032) # 0.005 # 1e-1                    # Target network update rate
		parser.add_argument("--policy_noise", default=0.2) # R:TD3 only              # Noise added to target policy during critic update
		parser.add_argument("--noise_clip", default=0.29) #0.5# R:TD3 only                # Range to clip target policy noise
