...
class AACHER(object):
    def __init__(self, state_dim, action_dim, max_action, discount, tau, hidden_sizes, num_actors=5, num_critics=5, actor_lr=3e-4, critic_lr=3e-4, smr_ratio=10, rc_weight=0.005, her_k=4):
        ...
        self.her_k = her_k  # Number of HER samples per experience
        self.actor_loss = 0
        self.critic_loss = 0
    ...
    def train(self, replay_buffer, batch_size=256):
        for _ in range(self.smr_ratio):  # Sample Multiple Reuse
            ...
            # HER: Modify the goal in a fraction of samples
            for _ in range(self.her_k):
                idx = np.random.randint(len(state))
                next_state[idx] = state[idx]  # Pretend the achieved goal is the desired goal
                reward[idx] = 0.0  # Reward should be maxed when goal is achieved
