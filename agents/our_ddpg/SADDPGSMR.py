...
class DDPG(object):
	def __init__(self, state_dim, action_dim, max_action, discount=0.99, tau=0.005, hidden_sizes=[256, 256]):  # discount = 0.99, tau 0.005
	...
	def train(self, replay_buffer, batch_size=256):  # batch_size = 256
		...
		for M in range(self.smr_ratio):
			with torch.no_grad():
				...
				next_Q = self.q_weight * torch.min(next_Q1_a1, next_Q2_a1) + (1-self.q_weight) * torch.max(next_Q1_a1, next_Q2_a1)
