...
def oscar_agent(state_dim, action_dim, actor_lr, critic_lr, gamma, tau, batch_size):
	    ...
	    for nWifi in [25]:
		    ...
		    run = wandb.init(project="my-awesome-project", tags = wtags,name="GREY")
		    ...
		    parser.add_argument("--discount", default=0.9) #0.99             # Discount factor
		    parser.add_argument("--tau", default=1e-1) # 0.005                    # Target network update rate
			    ...
			    with tqdm.trange(1, steps_per_ep+1) as t:
				    for step in t: #
					    ...
					    if done:
						    ...
						    episode_avg += episode_reward
						    break
			    logger.log_episode(episode_reward, logger.sent_mb/(simTime), episode)
			    env.close()
		    run.finish()
		    print("Average Reward:", episode_avg/12)
		    return episode_avg/12
