...
from agents.our_ddpg.DADDPGSMR import DDPG
...
def objective(trial):
    ...
    hidden_sizes = [trial.suggest_int('hidden_size_1', 128, 512), trial.suggest_int('hidden_size_2', 128, 512)]
    ...
    noise_clip = trial.suggest_uniform('noise_clip', 0.1, 0.5)
    discount = trial.suggest_uniform('discount', 0.80, 0.99)
    q_weight = trial.suggest_uniform('q_weight', 0.05, 0.3)
    regularization_weight = trial.suggest_uniform('regularization_weight', 0.001, 0.01)
    smr_ratio = trial.suggest_int('smr_ratio', 5, 15)
    ...
    run = wandb.init(project="my-awesome-project", tags = wtags, name="HYP")
